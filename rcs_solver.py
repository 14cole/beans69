...
def _normalize_segment_orientation(
    ...
    """
    Auto-flip closed contours to enforce CCW winding (outward normals).

    For closed segments (TYPE 3/4/5 with closed geometry), the normal direction
    determines which side is IPN1/plus vs IPN2/minus.  A CW-wound contour
    produces inward normals, silently swapping material assignments.  We detect
    this via the shoelace signed area and reverse the primitive chain when
    negative (CW).

    Open segments are left untouched — the user controls orientation explicitly.
    """

    if seg_type < 3:
        # TYPE 1 and 2 (PEC/IBC on exterior side) don't depend on winding.
        return point_pairs, ang_deg

    is_closed, area2 = _segment_closed_area2(point_pairs, meters_scale)
    if not is_closed:
        return point_pairs, ang_deg

    if area2 >= 0.0:
        # Already CCW — normals point outward.
        return point_pairs, ang_deg

    # CW winding detected — reverse the primitive chain.
    ...
    # Flip arc angle sign when reversing direction.
