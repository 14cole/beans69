...
class FMMOperator:
    ...
    def _build_near_matrix(self):
        ...
        # Regular pairs: try C extension for real k, fall back to Python.
        k_is_real = abs(complex(self.k).imag) < 1e-12 * abs(complex(self.k).real)
        c_lib = self._load_c_lib() if k_is_real else None

        for q_order, pairs in regular.items():
            if c_lib is not None:
                self._batch_near_c(c_lib, pairs, q_order)
            else:
    ...
    def _batch_near_c(self, lib, pairs, q_order):
        """Batch near-field using C extension (real k only)."""
        import ctypes
        ...
        nq = len(qt)
        qt_c = np.array(qt, dtype=np.float64)
        qw_c = np.array(qw, dtype=np.float64)

        obs_p0 = self.p0s[obs_idx].ravel().astype(np.float64)
        obs_seg = self.segs[obs_idx].ravel().astype(np.float64)
        obs_n = self.normals[obs_idx].ravel().astype(np.float64)
        obs_L = self.lengths[obs_idx].astype(np.float64)
        src_p0 = self.p0s[src_idx].ravel().astype(np.float64)
        src_seg = self.segs[src_idx].ravel().astype(np.float64)
        src_n = self.normals[src_idx].ravel().astype(np.float64)
        ...
        c_dp = ctypes.c_double * 1  # dummy for pointer type
        lib.compute_sk_blocks_batch_q(
            ctypes.c_int(P), ctypes.c_int(nq),
            qt_c.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            qw_c.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            obs_p0.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            obs_seg.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            obs_n.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            obs_L.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            src_p0.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            src_seg.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            src_n.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
